...
```{r}
# Show seeds so users can see reproducibility settings
cat(sprintf("Seeds — single-pop: %d | multi-pop: %d\n", seed, mp_seed))
```
...
```
### Attack rate and Reff (single-pop)
...
# Reff from simulated paths (helper)
...
### Per-million toggle, attack-rate matrix, Reff

```{r}
# Compare incidence scaling

plot_multi(simF, which = "incidence", group_style = "combined", per_million = TRUE)
plot_multi(simF, which = "incidence", group_style = "combined", per_million = FALSE)
